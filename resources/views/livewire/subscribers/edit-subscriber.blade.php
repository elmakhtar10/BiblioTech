    ...
    @if($showEditForm)
        ...
        <div class="modal-content" style="position: fixed; top: 10%; left: 50%; transform: translateX(-50%); width: 50%; background: #fff; padding: 20px; border-radius: 8px; z-index: 100;">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h5>Modifier un Abonné</h5>
                <button style="background:none; border:none; font-size:1.5rem;" wire:click="closeEditForm">&times;</button>
            </div>

            <form enctype="multipart/form-data" wire:submit.prevent="updateSubscriber">
                <input type="email" class="form-control mb-2" placeholder="Email" wire:model="email">
                ...
                <input type="text" class="form-control mb-2" placeholder="Telephone" wire:model="telephone">
                @error('telephone') <span class="text-danger">{{ $message }}</span> @enderror

                <input type="text" class="form-control mb-2" placeholder="Adresse" wire:model="adresse">
                @error('adresse') <span class="text-danger">{{ $message }}</span> @enderror

                <input type="file" class="form-control mb-2"  accept="image/*" wire:model="photo">
