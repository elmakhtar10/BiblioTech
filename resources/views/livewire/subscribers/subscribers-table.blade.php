                    ...
                    <table class="table table-hover align-middle mb-0">
                        ...
                        @foreach($subscribers as $subscriber)
                            <tr wire:key="{{$subscriber->id}}">
                                <td>
                                    <img src="{{ asset('storage/' . $subscriber->photo) }}"
                                         alt="Photo auteur"
                                ...
                                <td>{{$subscriber->adresse}}</td>
                                <td>
                                    @if($subscriber->status == "ACTIVE")
                                        <button class="btn btn-success btn-sm" wire:click="changeStatus({{$subscriber->id}})">
                                            {{$subscriber->status}}
                                        </button>
                                    @else
                                        <button class="btn btn-danger btn-sm" wire:click="changeStatus({{$subscriber->id}})">
                                            {{$subscriber->status}}
                                        </button>
                                    @endif
                                ...
                                <td class="text-end">
                                    {{-- Bouton EDIT --}}
                                    <button class="btn btn-primary btn-sm" wire:click="$dispatch('openForm', [{{ $subscriber->id }}])">
                                    ...
                                    {{-- Bouton Supprimer --}}
                                    <button class="btn btn-danger btn-sm" onclick="confirm('Voulez-vous vraiment supprimer le livre {{$subscriber->titre}} ?') || event.stopImmediatePropagation()" wire:click="delete({{$subscriber->id}})">
                                ...
                                <td>
                                    <a href="{{ route('historique.show', $subscriber->id) }}" class="btn btn-light">
